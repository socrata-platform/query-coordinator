...
object SoQLAnalysisDepositioner {
  ...
  private def depositionExpr[ColumnId,Type](expr: CoreExpr[ColumnId, Type]): CoreExpr[ColumnId, Type] = {
    expr match {
      ...
      case fc: FunctionCall[ColumnId, Type] => FunctionCall[ColumnId, Type](fc.function, fc.parameters.map(depositionExpr), fc.window)(NoPosition, NoPosition)
    }
  }
  ...
  private def depsoitionOptJoins[ColumnId, Type](joins: Seq[Join[ColumnId, Type]]) = {
    joins.map { join =>
      ...
      Join(join.typ, join.from.copy(subAnalysis = mappedSubAna), depositionExpr(join.on), false)
    }
  }
}

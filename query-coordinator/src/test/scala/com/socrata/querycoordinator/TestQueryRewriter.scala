...
class TestQueryRewriter extends TestBase with TestCompoundQueryRewriterBase {
  ...
  test("rewrite union") {
    val q =
      """SELECT number1, sum(number1) GROUP BY number1
          UNION ALL
         SELECT nn, sum(nn) FROM @tttt-tttt GROUP BY nn
        """
    val queryAnalysis = analyzeCompoundQuery(q)
    ...
    val rewrittenQueryAnalysis = analyzeRewrittenCompoundQuery(rewrittenQuery, rollups("ru1"))

    ruNameApplied should equal("ru1")
